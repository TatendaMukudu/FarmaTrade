...
// Every fresh signal, split into the caller's province and the national
...
export async function getFreshSignals(province: string) {
  ...
  return {
    local: signals.filter((s) => s.province === province),
    national: signals.filter((s) => s.province === null),
    total: signals.length,
  };
}

// Signals relevant to one party: their province plus the national picture,
// strongest first. Province-scoped rows outrank national ones at equal
...
export async function getSignalsFor(
  ...
  const signals = await prisma.marketSignal.findMany({
    where: {
      ...
      OR: [{ province: party.province }, { province: null }],
    },
    orderBy: [{ strength: "desc" }],
    take: limit * 3,
  });
  ...
  return signals
    .sort((a, b) => {
      const localA = a.province ? 1 : 0;
      const localB = b.province ? 1 : 0;
